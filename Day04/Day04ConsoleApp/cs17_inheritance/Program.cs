...
namespace cs17_inheritance
{
    ...
    class Parent
    {
        public string Name;     // 상속할때는 private 쓰면 안됨 protected 쓰면 됨
    ...
    class Child : Parent
    {
        public Child(string Name) : base(Name)      // 부모생성자를 먼저 실행한 뒤 자신 생성자를 실행
        {
    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            Child c = new Child("c");
            c.ParentMethod();   // 자식클래스는 부모클래스의 속성, 기능을 모두 쓸 수 있다.(단, public
            c.ChildMethod();
            ...
            // mammal.Bark();                  // 바로 안됨
            if (mammal is Dogs)                // Dogs를 생성해서 여기에 할당
            {
                Dogs midDog = mammal as Dogs;  //(Dogs)mammal; 형변환 할 때 as 사용 위의 is와 한 쌍
                midDog.Bark();
            }

            //Dogs dogs = new Mammal();        // 보모클래스가 자식클래스로 변환은 불가
            Dogs dog2 = new Dogs();
            Cats cat2 = new Cats();
