...
namespace wf13_bookrentalshop
{
    public partial class FrmBooks : Form
    {
        ...
        private void FrmGenre_Load(object sender, EventArgs e)
        {
            ...
            DtpRealeaseDate.Format = DateTimePickerFormat.Custom;
            DtpRealeaseDate.CustomFormat = "yyyy-MM-dd";
        }
        ...
        private void DgvResult_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex > -1) // 아무것도 선택안하면 -1
            {
                ...
                DtpRealeaseDate.Value = (DateTime)selData.Cells[5].Value;
        ...
        private void ClearInputs()
        {
            ...
            DtpRealeaseDate.Value = DateTime.Now;   // 오늘 날짜로 초기화
            NudPrice.Value = 0;
        ...
        private void LoadCboData()
        {
            try
            {
                using (MySqlConnection conn = new MySqlConnection(Commons.ConnString))
                {
                    ...
                    var temp = new Dictionary<string, string>();        // key에 대한 value
                    while (reader.Read())
                    {
                        temp.Add(reader[0].ToString(), reader[1].ToString());       // (key)B001, (value)공포/스릴러
                    }
                    //콤보박스에 할당
                    CboDivision.DataSource = new BindingSource(temp, null);  // null대신 "divtbl"가능
        ...
        private void SaveData()
        {
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(Helpers.Commons.ConnString))
                {
                    ...
                    if (isNew)
                    {
                        query = @"INSERT INTO bookstbl (Author,
                                                        Division,
                                                        Names,
                                                        ReleaseDate,
                                                        ISBN,
                    ...
                    MySqlParameter prmReleaseDate = new MySqlParameter("@ReleaseDate", DtpRealeaseDate.Value);
                    ...
                    if(isNew == false)  // update할 때는 boookIdx 파라미터를 추가
                    {
                        MySqlParameter prmBookIdx = new MySqlParameter("@bookIdx.Text", TxtBookIdx.Text);
